...
?>
        ...
        <article <?php post_class('grid-item item-s-12 item-l-6 item-xl-4 no-gutter grid-row margin-bottom-mid'); ?> id="post-<?php the_ID(); ?>">
          ...
          <div class="grid-item item-s-4 offset-s-1">
            <h2 class="margin-bottom-small font-size-medium font-bold"><a href="<?php the_permalink() ?>"><?php the_title(); ?></a></h2>
<?php
    if (!$in_stock) {
...
?>
        <article class="u-alert grid-item item-s-12"><h1 class="font-size-large"><?php _e('No products'); ?></h1></article>
<?php
} ?>
      </div>
    </div>
